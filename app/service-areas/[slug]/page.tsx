...
export default async function CityPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <Shell>
            ...
            <div className="overflow-hidden rounded-lg bg-card shadow-[var(--shadow-lg)]">
              ...
              <ul className="grid sm:grid-cols-2">
                {c.neighborhoods.map((n) => (
                  <li
                    key={n}
                    className="border-b border-border px-6 py-3 text-[0.95rem] text-foreground"
                  >
                    {n}
                  </li>
                ))}
            ...
            <div className="mt-10 overflow-hidden rounded-lg bg-primary shadow-[var(--shadow-dark)]">
              ...
              <ul className="grid sm:grid-cols-2 lg:grid-cols-4">
                {c.neighborhoods.map((n) => (
                  <li
                    key={n}
                    className="border-b border-on-dark/10 px-6 py-3.5 text-[0.95rem] text-on-dark-muted"
                  >
                    {n}
                  </li>
                ))}
      ...
      <PageCta city={c.name} omit={["/how-it-works"]} />
    </Shell>
  );
}
