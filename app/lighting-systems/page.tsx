 ...
 * So the centrepiece is that decision as a single table with a third column saying why each
...
const decision: { spec: string; sig: string; basic: string; why: string }[] = [
  ...
  },
  {
    ...
    sig: "RGB plus white",
    basic: "RGB",
    why: "Both do colour. Only the Signature tier does colour and true white from separate emitters.",
  },
  {
    spec: "Zones",
  ...
  },
  {
    ...
    basic: "Supported",
    why: "Per-zone dimming is how a colour stays clean rather than turning to a glare. It matters most on a low ranch elevation close to the street.",
  },
  {
    spec: "Channel finish",
    sig: "Colour matched to your trim",
...
const identical: string[] = [
  ...
  "Mitred corners at every gable, dormer, bay and valley",
...
const houses: { h: string; profile: string[]; verdict: string; slug: string }[] = [
  ...
  },
  {
    ...
    profile: [
      ...
      "Colour on gamedays and holidays, white the rest of the year",
    ],
    verdict:
...
export default function SystemsHub() {
  return (
    <Shell>
      ...
      <PageHero
        /* spec variant: the photograph and the scrim are the home page's, but the right
         * column is the decision rather than the form. This is a page somebody reads
         * before they are ready to book, and a form here would be premature. */
        variant="spec"
        ...
        aside={
          <div className="overflow-hidden rounded-lg bg-card shadow-[var(--shadow-lg)]">
            <div className="flex flex-wrap items-baseline justify-between gap-3 border-b border-border px-6 py-4">
              ...
              <p className="text-xs text-muted-foreground">Named on your quote</p>
            </div>
            <ul className="divide-y divide-border">
              {["brytr-signature", "brytr-basic"].map((sl) => {
                ...
                return (
                  <li key={sl} className="px-6 py-5">
                    <p className="label text-accent-ink">{s.tier}</p>
                    <h2 className="mt-1.5 font-display text-[1.15rem] font-bold text-foreground">
                      <Link href={`/lighting-systems/${sl}`} className="hover:text-accent-deep">
                        {s.name}
                      </Link>
                    </h2>
            ...
            <p className="border-t border-border bg-muted px-6 py-4 text-sm leading-relaxed text-muted-foreground">
              Everything else on this page bolts on to whichever of those two you pick.
            </p>
          </div>
        }
      />
      ...
      <section className="section bg-background">
        <div className="shell">
          <SectionHead
            ...
            title="Nine rows, and what each one actually costs you."
              ...
              {decision.map((r) => (
                <li key={r.spec} className="grid gap-3 px-6 py-5 lg:grid-cols-[20fr_23fr_20fr_37fr] lg:gap-8">
                  ...
                  {/* the recommended column is marked by a SURFACE shift and the amber
                    * tick in the header, not by a coloured strip down the side of the cell.
                    * slopcheck flags the strip, and it is right to: a coloured left border
      ...
      <section className="section bg-muted">
        <div className="shell">
          <SectionHead
            ...
            title="Eight pages, but only one of them is a choice."
      ...
      <section className="section bg-primary">
        <div className="shell">
          <SectionHead
            ...
            title="Three houses, and what we would put on each."
